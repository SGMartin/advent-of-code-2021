...
def get_bingo_board(numbers, boards) -> dict:
    ...
    for bid, board in enumerate(boards):
        ...
        for nid, number in enumerate(numbers):
            ...
            res = (number == board)
            logic_board = logic_board + res ## Bolean sum over same size np.arrays = logic |
...
boards = np.split(ary = all_boards,
                  indices_or_sections = len(all_boards) / 5)


winning_combinations = get_bingo_board(numbers = numbers_called,
                                       boards = boards)
...
best_board_id = min(winning_combinations,
                    key=lambda k:winning_combinations[k][0])
